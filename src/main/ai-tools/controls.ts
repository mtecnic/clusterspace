 ...
 * renderer consumes and applies to the target pane, and awaits an ack
 * reporting whether a registered handler actually existed for pane_id/tab_id
 * (see pane-control-ack.ts) — a stale/hallucinated pane_id now gets a real
 * error instead of an unconditional "success". All are `write_local` risk
 * (see goal-policy BUILTIN_PERMISSIONS) so they run without approval prompts.
 */
export function registerControlTools(): void {
  toolRegistry.register<{ pane_id: string; tab_id: string }, { success: boolean; error?: string }>({
    ...
    run: async ({ pane_id, tab_id }, { window }) => {
      const ok = await sendPaneControl(window, IPC_CHANNELS.AI_SWITCH_TERMINAL_TAB, { paneId: pane_id, tabId: tab_id })
      return ok
        ? { success: true }
        : { success: false, error: `No registered tab "${tab_id}" on pane ${pane_id} — check list_panes for current pane/tab ids.` }
    }
  })

  toolRegistry.register<{ pane_id: string; url?: string }, { success: boolean; error?: string }>({
    ...
    run: async ({ pane_id, url }, { window }) => {
      const ok = await sendPaneControl(window, IPC_CHANNELS.AI_BROWSER_TAB_ACTION, { paneId: pane_id, action: 'open', url })
      return ok
        ? { success: true }
        : { success: false, error: `No registered browser pane with id ${pane_id} — check list_panes, or convert_pane_to_browser first.` }
    }
  })
  ...
  toolRegistry.register<{ pane_id: string; tab_id: string }, { success: boolean; error?: string }>({
    ...
    run: async ({ pane_id, tab_id }, { window }) => {
      const ok = await sendPaneControl(window, IPC_CHANNELS.AI_BROWSER_TAB_ACTION, { paneId: pane_id, action: 'switch', tabId: tab_id })
      return ok
        ? { success: true }
        : { success: false, error: `No registered tab "${tab_id}" on browser pane ${pane_id} — check list_panes for current pane/tab ids.` }
    }
  })
  ...
  toolRegistry.register<{ pane_id: string; tab_id: string }, { success: boolean; error?: string }>({
    ...
    run: async ({ pane_id, tab_id }, { window }) => {
      const ok = await sendPaneControl(window, IPC_CHANNELS.AI_BROWSER_TAB_ACTION, { paneId: pane_id, action: 'close', tabId: tab_id })
      return ok
        ? { success: true }
        : { success: false, error: `No registered tab "${tab_id}" on browser pane ${pane_id} — check list_panes for current pane/tab ids.` }
    }
  ...
  toolRegistry.register<{ pane_id: string; tab_id?: string }, { success: boolean; error?: string }>({
    ...
    run: async ({ pane_id, tab_id }, { window }) => {
      ...
      const ok = await sendPaneControl(window, IPC_CHANNELS.AI_RECONNECT_PANE, { paneId: pane_id, tabId: tab_id })
      return ok
        ? { success: true }
        : { success: false, error: `No registered ${isBrowser ? 'browser' : 'terminal'} pane/tab for ${pane_id}${tab_id ? ` tab ${tab_id}` : ''} — it may not be mounted in the live UI. Check list_panes for current ids.` }
    }
  })
}
